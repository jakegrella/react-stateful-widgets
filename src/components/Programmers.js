...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return awesome[activeAwesome].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: (activeAwesome != null) ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          awesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setActiveAwesome(dev.id - 1)}}>Feature</button>
            </div>
          )
        }
      </div>
